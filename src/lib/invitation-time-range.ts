...
export const EVENT_DAY_INVITATION_TIME_RANGES: Record<string, string> = {
...
export function getEventDayInvitationTimeRange(dayId: EventDayId): string {
  return EVENT_DAY_INVITATION_TIME_RANGES[dayId] ?? DEFAULT_INVITATION_TIME_RANGE;
}
...
export function invitationTimeRangeForEventDays(
  ...
  if (sorted.length === 1) {
    return EVENT_DAY_INVITATION_TIME_RANGES[sorted[0]] ?? DEFAULT_INVITATION_TIME_RANGE;
  }
  return sorted
    .map(
      (id) =>
        `${getEventDayById(id).label} : ${EVENT_DAY_INVITATION_TIME_RANGES[id] ?? DEFAULT_INVITATION_TIME_RANGE}`,
    )
    .join(" · ");
}
