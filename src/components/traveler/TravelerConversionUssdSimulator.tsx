...
import {
  applyTravelerConversionUssdChoice,
  formatVolumeKeypadDisplay,
  getTravelerConversionUssdScreenView,
  INITIAL_TRAVELER_CONVERSION_USSD_STATE,
  type TravelerConversionUssdState,
} from "@/lib/traveler-ussd/conversion-simulation";
...
export function TravelerConversionUssdSimulator({ onClose, onComplete }: Props) {
  const [sim, setSim] = useState<TravelerConversionUssdState>(
    INITIAL_TRAVELER_CONVERSION_USSD_STATE,
  );
  ...
  const view = getTravelerConversionUssdScreenView(sim);
  const usesKeypad = view.inputMode === "keypad";
  const canReply = view.inputMode !== "none";
  const keypadDisplayValue =
    sim.screen === "volume" ? formatVolumeKeypadDisplay(input) : input;
  ...
  const applyChoice = useCallback(
    (choice: string) => {
      ...
      const { state: next, accepted, result } = applyTravelerConversionUssdChoice(
        prev,
        choice,
      );
      ...
      if (!accepted) {
        setStatusMsg(
          prev.screen === "volume"
            ? "Volume invalide — saisissez une quantité (ex. 1, 3, 05 pour 0.5)"
            : "Choix invalide",
        );
      ...
      if (result.type === "complete") {
        setStatusMsg("Conversion confirmée…");
        window.setTimeout(() => {
          onComplete?.();
          onClose?.();
        }, 700);
    ...
    <div className="mx-auto flex h-full min-h-0 w-full max-w-2xl flex-1 flex-col sm:max-w-xl">
      <div className="flex min-h-0 flex-1 flex-col px-4 py-4 sm:px-6 sm:py-5">
        <section
          ...
          aria-label="Menu USSD conversion forfait"
        >
          <div
          ...
          <div
            ref={screenRef}
            key={`${sim.screen}-${sim.conversionLabel ?? "menu"}`}
        ...
        {replyOpen && usesKeypad && (
          <div ref={keypadRef} className="mt-3 will-change-transform">
            <UssdKeypad
              ...
              onChange={(digit) => {
                setInput((prev) => {
                  if (simRef.current.screen === "volume") {
                    if (!digit) return "";
