...
import {
  applyPrivilegeUssdChoice,
  getPrivilegeUssdScreenView,
...
export function PrivilegeUssdPhoneSimulator({
  ...
  const view = getPrivilegeUssdScreenView(sim);
    ...
    <div className="mx-auto flex h-full min-h-0 w-full max-w-2xl flex-1 flex-col sm:max-w-xl">
      <div className="flex min-h-0 flex-1 flex-col px-4 py-4 sm:px-6 sm:py-5">
        <section
          ...
          aria-label="Menu USSD Privilege"
        >
          <div
          ...
          <div
            ref={screenRef}
            key={`${sim.screen}-${sim.stubLabel ?? "root"}`}
        ...
        {replyOpen && view.showInput && (
          <div ref={keypadRef} className="mt-3 will-change-transform">
            <UssdKeypad
              ...
              onChange={(digit) => {
                setInput((prev) => {
                  if (simRef.current.screen !== "root") return digit;
                  if (prev === "1" && digit === "0") return "10";
                  return digit;
                });
                setStatusMsg(null);
              }}
