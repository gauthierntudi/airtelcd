...
import {
  applyPrivilegeOptInChoice,
  getPrivilegeOptInScreenView,
  INITIAL_PRIVILEGE_OPT_IN_STATE,
  type PrivilegeOptInState,
} from "@/lib/privilege-opt-in-ussd/simulation";
import { MPESA_USSD_COLORS as C } from "@/lib/mpesa-ussd/theme";
import { PRIVILEGE_DIAL_CODE } from "@/lib/privilege-onboarding";
...
export function PrivilegeOptInUssdSimulator({
  onClose,
  onCancel,
  onConfirmed,
}: Props) {
  const [sim, setSim] = useState<PrivilegeOptInState>(
    INITIAL_PRIVILEGE_OPT_IN_STATE,
  );
  ...
  const view = getPrivilegeOptInScreenView(sim);
  ...
  const applyChoice = useCallback(
    (choice: string) => {
      ...
      const { state: next, accepted, result } = applyPrivilegeOptInChoice(
        prev,
        choice,
      );
      if (!accepted) {
    ...
    <div className="mx-auto flex h-full min-h-0 w-full max-w-2xl flex-1 flex-col sm:max-w-xl">
      <div className="flex min-h-0 flex-1 flex-col px-4 py-4 sm:px-6 sm:py-5">
        <section
          ...
          aria-label="Menu USSD Vodacom Privilège"
        >
          <div
          ...
          >
            ...
            >
              {PRIVILEGE_DIAL_CODE} — Vodacom Privilège
            </p>
          </div>
      ...
      >
        <div className="grid w-full grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-3 sm:gap-4">
          ...
          <button
            ...
            className="flex min-h-[56px] w-full min-w-0 items-center justify-center rounded-2xl py-4 font-vodafone-exb text-base tracking-wide text-white shadow-[0_6px_20px_rgba(230,0,0,0.45)] active:scale-[0.98] disabled:opacity-45 sm:min-h-[60px] sm:text-lg"
